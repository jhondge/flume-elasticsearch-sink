...
public class TestNginxIndexBuilder {

    private NginxIndexBuilder headerBasedIndexBuilder;
    ...
    @Before
    public void init() {
        headerBasedIndexBuilder = new NginxIndexBuilder();
    }
...
//        context.put(ES_INDEX, "index-${yy}");
        context.put(ES_INDEX, "index-${index_name}-${yy}");
        context.put(ES_TYPE, type);
        headerBasedIndexBuilder.configure(context);

        Map<String, String> headers = new HashMap<>();
        headers.put("index_name", index);
        headers.put("yy", "2018");
        headers.put(TYPE, type);
        event.setHeaders(headers);
        assertEquals("index-" + index + "-2018", headerBasedIndexBuilder.getIndex(event));
